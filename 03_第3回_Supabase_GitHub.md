...
今回は2つのサービスを使いますが、まず **GitHub** のアカウントを作ります。GitHub アカウントは Supabase のログインにも、第4回の Vercel にも使うので、全ての土台になります。
...
- 「いつ、何を変えたか」の履歴が全部残る
- 壊しても過去の状態に戻せる
- Supabase のログイン、第4回の Vercel デプロイにも使う

> 💡 **なぜ GitHub を選ぶのか？**
> コード管理サービスは GitLab や Bitbucket もありますが、GitHub は世界最大のプラットフォームで、Supabase のログインにも Vercel の連携にも GitHub アカウントが必要です。つまり GitHub を使えば全てが繋がります。
...
- 管理画面が直感的で初心者にやさしい
- Claude Code との相性が良い
- **ログインには GitHub アカウントを使います**（Step 1 で作成済み）

> 💡 **なぜ Supabase を選ぶのか？**
> データベースサービスは他にもあります（Firebase、PlanetScale など）。Supabase を選んだ理由は3つです:
> 1. **無料プランが十分** — 個人開発なら課金不要
> 2. **管理画面が直感的** — テーブルの中身を Excel のように確認できる
> 3. **Claude Code との相性が良い** — 接続設定をAIに任せられる
...
### Step 1: GitHub アカウント作成（10分）

> **このStepでやること:** GitHub のアカウントを作成します。この後の Supabase ログイン、第4回の Vercel にも使う大切なアカウントです。
...
*GitHub 公式サイト。「Sign up」からアカウント作成に進みます*
...
7. パズル認証を完了
8. メールに届いた確認コードを入力
9. プラン選択画面が出たら「Free」を選択（無料で十分です）

> 💡 **ユーザー名の選び方**
> GitHub のユーザー名は公開されます。本名でもニックネームでもOKですが、仕事で使う可能性があるなら「名前のローマ字」がおすすめです（例: taro-yamada）。

既にアカウントをお持ちの方は、https://github.com にログインできることを確認して次に進んでください。

---

### Step 2: Supabase のセットアップ（15分）

> **このStepでやること:** Step 1 で作った GitHub アカウントで Supabase にログインし、TODO データを保存するためのデータベースとテーブルを準備します。
...
### Step 3: TODO アプリに Supabase を接続する（20分）
...
### Step 4: GitHub にコードをバックアップする（15分）

> **このStepでやること:** 書いたコードを Step 1 で作った GitHub にアップロードして、安全にバックアップします。あわせて「秘密の情報をアップしない」設定も行います。
